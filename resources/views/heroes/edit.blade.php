...
<head>
    ...
    <style>
        ...
        .item-picker-dropdown {
            display: none; position: absolute; top: 105%; left: 0; width: 600px; background: white; border: 1px solid #d1d5db; border-radius: 12px;
            box-shadow: 0 10px 25px -5px rgba(0, 0, 0, 0.1), 0 8px 10px -6px rgba(0, 0, 0, 0.1);
            z-index: 1050; overflow: hidden; height: 320px;
        }
        ...
        .item-card { border: 1px solid #e5e7eb; border-radius: 8px; padding: 8px; text-align: center; cursor: pointer; transition: all 0.2s; position: relative; }
        ...
        .selected-badge { background: #fff; border: 1px solid #d1d5db; border-radius: 6px; padding: 4px 10px 4px 6px; font-size: 0.85rem; font-weight: 600; display: inline-flex; align-items: center; gap: 8px; }
...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-lg-10">
            <div class="card">
                <div class="card-header"><h4>Edit Hero: {{ $hero->name }}</h4></div>
                <div class="card-body p-5">
                    @if ($errors->any()) <div class="alert alert-danger mb-4"><ul class="mb-0 small">@foreach ($errors->all() as $e) <li>{{ $e }}</li> @endforeach</ul></div> @endif
                    ...
                    <form action="{{ route('heroes.update', $hero->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row g-5">
                            <div class="col-md-5">
                                ...
                                <div class="mb-4"><label>Nama Hero</label><input type="text" name="name" class="form-control" value="{{ old('name', $hero->name) }}" required></div>
                                <div class="mb-4">
                                    <label>Ganti Foto</label><input type="file" name="photo" class="form-control" accept="image/*">
                                    <div class="mt-3"><img src="{{ \Illuminate\Support\Str::startsWith($hero->photo, 'images/') ? asset($hero->photo) : asset('storage/' . $hero->photo) }}" width="80" class="rounded"></div>
                                </div>
                                
                                <div class="mb-4">
                                    <label>Role</label>
                                    <div class="item-picker-container picker-no-cat" id="rolePicker">
                                        <div class="item-picker-trigger" id="roleTrigger"><span class="placeholder-text">Pilih Role...</span></div>
                                ...
                                <div class="mb-4">
                                    ...
                                    <div class="item-picker-container picker-no-cat" id="posPicker">
                                        <div class="item-picker-trigger" id="posTrigger"><span class="placeholder-text">Pilih Posisi...</span></div>
                            ...
                            <div class="col-md-7">
                                ...
                                <div class="mb-4">
                                    ...
                                    <div class="item-picker-container" id="itemPicker">
                                        <div class="item-picker-trigger" id="pickerTrigger"><span class="placeholder-text">Klik untuk pilih item...</span></div>
                                ...
                                <div class="mb-4"><label>Kisah / Lore</label><textarea name="story" id="summernote" required>{{ old('story', $hero->story) }}</textarea></div>
                            </div>
                        </div>
                        ...
                        <div class="d-flex gap-3 mt-5 pt-4 border-top justify-content-end">
                            ...
                            <button type="submit" class="btn btn-primary">Perbarui Hero</button>
                        </div>
                    </form>
                </div>
            </div>
...
<script>
    $(document).ready(function() {
        $('#summernote').summernote({
            placeholder: 'Tulis legenda hero...', tabsize: 2, height: 350,
